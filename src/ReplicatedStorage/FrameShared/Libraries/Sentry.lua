...
local sdkName = "cloud-frame"
local sdkVersion = "2.0"
...
function Raven:SendException(eType, errorMessage, traceback, config, eventLevel)
	...
	local packet = {
		level = eventLevel or Raven.EventLevel.Error;
		culprit = culprit;
	}
...
function Raven:ConnectRemoteEvent(remoteEvent)
	assert(typeof(remoteEvent) == "Instance" and remoteEvent.ClassName == "RemoteEvent", "ConnectRemoteEvent did not receive RemoteEvent instance")
	
	remoteEvent.OnServerEvent:Connect(function(player, errorMessage, traceback, options)
		...
		if (count > 0) then
			if (type(errorMessage) == "string" and (type(traceback) == "string" or traceback == nil)) then
				...
				if (success) then
					count = count - 1
					self:SendException(Raven.ExceptionType.Client, scrubbedErrorMessage, scrubbedTraceback, options)
				else
					if (logWarnings) then
